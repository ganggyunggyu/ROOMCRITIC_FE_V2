...
export const Card: React.ForwardRefExoticComponent<CardProps & React.RefAttributes<HTMLDivElement>> = React.forwardRef(
  ({ content }, ref) => {
    ...
    return (
      ...
      >
        ...
        <div className={`text-center flex gap-2 flex-col md:w-[card-img-w] w-[card-img-sm-w]`}>
          ...
          <CardInfo title={content.title} grade={(content.voteAverage / 2).toFixed(2)} etc={formattedMonthEnd} />
        </div>
      </figure>
    );
  },
