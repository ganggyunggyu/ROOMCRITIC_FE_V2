export const BANNER_IMAGES = [
  'https://www.themoviedb.org/t/p/original/j29ekbcLpBvxnGk6LjdTc2EI5SA.jpg',
  'https://an2-img.amz.wtchn.net/image/v2/Zi2FmJJFiKq7ZP1OEf3JGA.jpg?jwt=ZXlKaGJHY2lPaUpJVXpJMU5pSjkuZXlKdmNIUnpJanBiSW1SZk1Ua3lNSGd4TURnd2NUZ3dJbDBzSW5BaU9pSXZkakl2YzNSdmNtVXZhVzFoWjJVdk1UWTBORFk0T1RjNE5qZ3pOVGN5TURZM01DSjkudjg3TUgtTDBWXzdJRUdURFZ2d2JENUhldXdfa0NyLUlKdUFvbmFCc2tlOA',
  'https://www.themoviedb.org/t/p/original/lxD5ak7BOoinRNehOCA85CQ8ubr.jpg',
];
export const BANNER_REVIEWS = [
  {
    title: '인사이드 아웃',
    lines: [
      '나는 이제 너에게도 슬픔을 주겠다.',
      '사랑보다 소중한 슬픔을 주겠다.',
      "(정호승의 시 '슬픔이 기쁨에게')",
    ],
    link: '/content/movie/65dbf74e13f52a0d8acd8a21',
    movie_color: 'rgb(196 181 253)',
    hover_color: 'rgb(109 40 217)',
  },
  {
    title: '라라랜드',
    lines: ['달콤 쌉사름한 그 모든 감정에', '화룡점정하는 마법같은 순간.'],
    link: '/content/movie/65dbf75913f52a0d8acd90b4',
    movie_color: 'rgb(253 164 175)',
    hover_color: 'rgb(190 18 60)',
  },
  {
    title: '토이스토리',
    lines: ['넌 나의 친구야,', '영원한 친구야.'],
    link: '/content/movie/65dbf74e13f52a0d8acd8a2c',
    movie_color: 'rgb(134 239 172)',
    hover_color: 'rgb(21 128 61)',
  },
];
